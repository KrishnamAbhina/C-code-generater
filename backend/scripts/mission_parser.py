...
if __name__ == "__main__":
    try:
        ...
        if mission_data and dop_dip_data:
            ...
            output_json_path = "final_mission_parsing_output.json"
            save_final_structured_output(transformed_data, dop_dip_data)

            print(f"✔ JSON written to {output_json_path}")

            # ✅ Automatically run Code 2: c_code_generation.py
            script_path = os.path.join(os.path.dirname(__file__), "c_code_generation.py")
            print("🔄 Launching C code generator...")
            subprocess.run(["python", script_path, output_json_path], check=True)
